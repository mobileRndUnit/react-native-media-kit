...
export default class Controls extends React.Component {
  ...
  render() {
    ...
    let bufferIndicator;
    if (this.props.buffering && this.props.showIndicator) {
      bufferIndicator = (
        <ActivityIndicator
          color={'#f2f2f2'}
          size={'large'} />
    ...
    if (this.props.currentFormated && this.props.totalFormated) {
      currentFormated = this.props.currentFormated;
      totalFormated = this.props.totalFormated;
    }
    ...
    if (this.props.playing && this.props.paused) {
      playOrPause = (
        <TouchableOpacity
          ...
          <Image
            style={{ width: 20, height: 20, resizeMode: 'contain' }}
            source={!this.props.paused ? require('./img/media-player-pause.png') : require('./img/media-player-play.png')} />
        </TouchableOpacity>
      );
    }
    ...
    if (this.props.overlay) {
      ...
      mainStyle = { position: 'absolute', left: 0, right: 0, bottom: 0, height: 40, backgroundColor: '#00000033', flexDirection: 'row' };
    }

    if (this.props.style) {
      rootStyle = this.props.style.rootStyle;
      mainStyle = this.props.style.mainStyle;
    }
